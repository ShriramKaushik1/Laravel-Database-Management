            ...
            @foreach($duplicateGroups as $index => $group)
                <div class="bg-gray-50/50 rounded-xl border border-gray-200 overflow-hidden shadow-sm">
                    <div class="px-4 py-3 border-b border-gray-200 bg-gray-100/50 flex justify-between items-center">
                        <span class="font-semibold text-gray-700">Group #{{ $index + 1 }} - {{ $group[0]->business_name }} ({{ count($group) }} duplicates)</span>
                    </div>
                    
                    <form method="POST" action="{{ route('deduplicate.merge', $group[0]->id) }}" class="p-4">
                        @csrf
                        <div class="overflow-x-auto mb-4">
                            <table class="min-w-full divide-y divide-gray-200 text-sm focus:outline-none">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach($group as $dupKey => $record)
                                        <tr class="{{ $dupKey == 0 ? 'bg-indigo-50/30' : '' }}">
                                            <td class="px-3 py-2">
                                                <input type="checkbox" name="duplicate_ids[]" value="{{ $record->id }}" checked class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" {{ $dupKey == 0 ? 'disabled' : '' }}>
                                                @if($dupKey == 0)
                                                    <input type="hidden" name="duplicate_ids[]" value="{{ $record->id }}">
                                                    <span class="text-xs text-indigo-600 block mt-1">Primary</span>
                                                @endif
                                            </td>
                                            <td class="px-3 py-2 font-medium">{{ $record->business_name }}</td>
                                            <td class="px-3 py-2 text-gray-500">{{ $record->area }}</td>
                                            <td class="px-3 py-2 text-gray-500">{{ $record->city }}</td>
                                            <td class="px-3 py-2 text-gray-500">{{ $record->address }}</td>
                                            <td class="px-3 py-2 text-gray-500">{{ $record->mobile_no }}</td>
                                            <td class="px-3 py-2 text-gray-500">{{ $record->category }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
